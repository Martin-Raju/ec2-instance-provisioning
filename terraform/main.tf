...
locals {
  alb_arn = var.create_alb
    ? module.alb[0].lb_arn
    : try(data.aws_lb.existing[0].arn, null)

  alb_target_group_arn = var.create_alb
    ? module.alb[0].target_group_arns[0]
    : try(data.aws_lb_target_group.existing[0].arn, null)
}
